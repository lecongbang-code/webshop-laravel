...
<script type="text/javascript">
  ...
  function loadMoreProductDiscount(id = '') {
    $.ajax({
      url:'{{url('/load-more-product-discount-code')}}',
      method:"POST",
      headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
      data:{id:id},
      success:function(data){
        $('#btn_remove_discount_code').remove();
        $('#all_discount_code').append(data);
      }
    });
  }

  function myFunction(){
    var id = $("#load_more_product_discount_code").data("sample-id");
    loadMoreProductDiscount(id);
  }
  ...
  <section id="portfolio" class="portfolio" style="padding-bottom: 20px; padding-top: 20px;">
    <div class="container">
      ...
      <div class="row" id="all_discount_code">
    ...
    <div class="container" id="btn_remove_discount_code">
      <div style="margin: auto;  width: 200px; text-align: center;">
        <button id="load_more_product_discount_code" class="btn btn-primary" onclick="myFunction()" data-sample-id="{{$last_id}}">Xem thêm</button>
      </div>
    </div>
