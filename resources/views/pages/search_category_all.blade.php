...
<script type="text/javascript">
  ...
  function loadMoreProduct(id = '') {
    $.ajax({
      url:'{{url('/load-more-product3')}}',
      method:"POST",
      headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
      data:{id:id},
      success:function(data){
        $('#btn_remove2').remove();
        $('#all_product2').append(data);
      }
    });
  }

  function clickMoreProduct2() {
    var id = document.getElementById("load_more_button2").name;
    loadMoreProduct(id);
  }
</script>
