@extends('index')
  ...
  <script type="text/javascript">
    ...
    function loadMoreCategoryProduct(id = '',parameter = '') {
      $.ajax({
        url:'{{url('/load-more-category-product')}}',
        method:"POST",
        headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        data:{id:id,parameter:parameter},
        success:function(data){
          $('#btn_remove_category1').remove();
          $('#all_product_category1').append(data);
        }
      });
    }

    function loadMoreCategoryProduct2(id = '') {
      $.ajax({
        url:'{{url('/load-more-product2')}}',
        method:"POST",
        headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        data:{id:id},
        success:function(data){
          $('#btn_remove_category2').remove();
          $('#all_product_category2').append(data);
        }
      });
    }

    function myFunction(){
      var id = $("#load_more_button_category1").data("sample-id");
      var parameter = $("#load_more_button_category1").data("sample-par");
      loadMoreCategoryProduct(id,parameter);
    }

    function myFunction2(){
      var id = $("#load_more_button_category2").data("sample-id");
      loadMoreCategoryProduct2(id);
    }
  ...
  @section('index_content')
    ...
    <section id="portfolio" class="portfolio" style="padding-bottom: 20px; padding-top: 20px;">
      <div class="container">
        ...
        <div class="row">
          <div class="col-lg-12">
            <ul id="portfolio-flters">
              <a href="{{URL::to('/search-category-all')}}">
                <li data-filter="*" class="@php
              if(request()->path() == 'home' || request()->path() == 'search/list-product-discount-code' 
              || request()->path() == 'search/super-discount' 
              || request()->path() == 'search-product' 
              || request()->path() == 'search-category-all' 
              || request()->path() == '/')
              {
                  echo "filter-active";
              }
        ...
        <div class="row" id="all_product_category1">

          @foreach($list_product as $key => $product)

          <div class="col-lg-3 col-md-6 portfolio-item filter-{{$product->category_id}} wow fadeInUp list-product" 
            style="height: 480px; padding-right: 5px; padding-left: 5px; <?php if('search-category/'.$product->category_id != request()->path())echo 'display: none;';?>">
  ...
  @section('product_more')
    ...
    <section id="portfolio" class="portfolio" style="padding-bottom: 20px; padding-top: 20px;">
      <div class="container">
        ...
        <div class="row" id="all_product_category2">
        ...
        <div id="btn_remove_category2" class="container">
          <div style="margin: auto;  width: 200px; text-align: center;">
              <button type="button" style="margin-top: 30px" id="load_more_button_category2" class="btn btn-primary" style="margin-top: 30px" data-sample-id="{{$last_id2}}" onclick="myFunction2()"> Xem thêm </button>
            </div>
        </div>
